
package com.bitmakers.firstjobs.model_class;

public class CompanyInfo {

    private String[] com_benefit;
    private String com_linked_in;
    private String com_desc;
    private String com_website;
    private String com_facebook;
    private String com_logo;
    private String com_district;
    private String id_company;
    private String com_intro;
    private String com_address;
    private String com_size;
    private String com_country;
    private String com_phone;
    private String[] company_image;
    private String com_name;
    private String com_city;
    private String lng;
    private String lat;
    private String com_who_we_are;
    private String formatted_address;
    private String com_thumbnail;

    public String[] getCom_benefit ()
    {
        return com_benefit;
    }

    public void setCom_benefit (String[] com_benefit)
    {
        this.com_benefit = com_benefit;
    }

    public String getCom_linked_in ()
    {
        return com_linked_in;
    }

    public void setCom_linked_in (String com_linked_in)
    {
        this.com_linked_in = com_linked_in;
    }

    public String getCom_desc ()
    {
        return com_desc;
    }

    public void setCom_desc (String com_desc)
    {
        this.com_desc = com_desc;
    }

    public String getCom_website ()
    {
        return com_website;
    }

    public void setCom_website (String com_website)
    {
        this.com_website = com_website;
    }

    public String getCom_facebook ()
    {
        return com_facebook;
    }

    public void setCom_facebook (String com_facebook)
    {
        this.com_facebook = com_facebook;
    }

    public String getCom_logo ()
    {
        return com_logo;
    }

    public void setCom_logo (String com_logo)
    {
        this.com_logo = com_logo;
    }

    public String getCom_district ()
    {
        return com_district;
    }

    public void setCom_district (String com_district)
    {
        this.com_district = com_district;
    }

    public String getId_company ()
    {
        return id_company;
    }

    public void setId_company (String id_company)
    {
        this.id_company = id_company;
    }

    public String getCom_intro ()
    {
        return com_intro;
    }

    public void setCom_intro (String com_intro)
    {
        this.com_intro = com_intro;
    }

    public String getCom_address ()
    {
        return com_address;
    }

    public void setCom_address (String com_address)
    {
        this.com_address = com_address;
    }

    public String getCom_size ()
    {
        return com_size;
    }

    public void setCom_size (String com_size)
    {
        this.com_size = com_size;
    }

    public String getCom_country ()
    {
        return com_country;
    }

    public void setCom_country (String com_country)
    {
        this.com_country = com_country;
    }

    public String getCom_phone ()
    {
        return com_phone;
    }

    public void setCom_phone (String com_phone)
    {
        this.com_phone = com_phone;
    }

    public String[] getCompany_image ()
    {
        return company_image;
    }

    public void setCompany_image (String[] company_image)
    {
        this.company_image = company_image;
    }

    public String getCom_name ()
    {
        return com_name;
    }

    public void setCom_name (String com_name)
    {
        this.com_name = com_name;
    }

    public String getCom_city ()
    {
        return com_city;
    }

    public void setCom_city (String com_city)
    {
        this.com_city = com_city;
    }


    public String getCom_who_we_are ()
    {
        return com_who_we_are;
    }

    public void setCom_who_we_are (String com_who_we_are)
    {
        this.com_who_we_are = com_who_we_are;
    }



    public String getCom_thumbnail ()
    {
        return com_thumbnail;
    }

    public void setCom_thumbnail (String com_thumbnail)
    {
        this.com_thumbnail = com_thumbnail;
    }

    public String getLng ()
    {
        return lng;
    }

    public void setLng (String lng)
    {
        this.lng = lng;
    }

    public String getLat ()
    {
        return lat;
    }

    public void setLat (String lat)
    {
        this.lat = lat;
    }


    public String getFormatted_address ()
    {
        return formatted_address;
    }

    public void setFormatted_address (String formatted_address)
    {
        this.formatted_address = formatted_address;
    }
}

